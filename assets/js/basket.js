...
window.addEventListener('click', function (event) {
    if (event.target.hasAttribute('data-cart') || event.target.classList.contains('basketAdd')) {
        ...
        if (itemInCart) {
            ...
            const cartItemHTML = `
                <li class = "cart-item" data-id="${productInfo.id}"  name = "product_id[]">
                    <div class="cart-img">
                        <a href="#"><img src="${productInfo.imgSrc}" alt=""></a>
                    </div>
                    <div class="cart-title">
                        <h4 name = "product_name[]">${productInfo.title}</h4>
                        <div class="product-items">
                            <div class="product-quality">
                                <div class="dec qtybutton" data-action = "minus">-</div>
                                    <span class="items-curret" data-counter="" name = "product_counter[]">${productInfo.counter}</span>
                                <div class="inc qtybutton" data-action = "plus">+</div>
                            </div>
                            <span class="prodPrice" name = "product_price[]">${productInfo.price}</span>
                        </div>
